...
if os.getenv("DOCKER") == "1":
    ...
    LOGGER.debug(f"DISPLAY=%s", os.environ['DISPLAY'])
...
def some_where_random_close(x, y, max_dist=120):
    ...
    while True:
        ...
        if x + rand_x in range(0, shape.width) and y + rand_y in range(0, shape.height):
...
def human_typing(text, speed=(0.01, 0.025), double_press=False):
    """
    ...
    while i <= len(text):
        ...
        if double_press is True and random.random() < 0.3 and i + 1 < len(text):
...
def type_write(to_type: Union[str, List]):
    pyautogui.typewrite(message=to_type, interval=0.22)
